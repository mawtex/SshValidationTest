...
{
    private static readonly MemberSetter CacheabilitySetter;
    ...
    static Renderers_Page()
    {
        var fieldInfo = typeof(HttpCachePolicy).GetField("_cacheability", BindingFlags.NonPublic | BindingFlags.Instance);
        CacheabilitySetter = fieldInfo.DelegateForSetFieldValue();
    }
    ...
    protected void CacheRequest()
    {
        // ----------------- This is a workaround to avoid no caching on https issue
        if (Request.IsSecureConnection
            && _renderingContext.Page.DataSourceId.PublicationScope == Composite.Data.PublicationScope.Published
            && !Composite.C1Console.Security.UserValidationFacade.IsLoggedIn())
        {
            CacheabilitySetter.Invoke(Response.Cache, HttpCacheability.ServerAndNoCache);
        }
